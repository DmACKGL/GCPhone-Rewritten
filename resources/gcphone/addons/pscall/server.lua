...
function notifyAlertSMS (number, alert, listSrc)
  if PhoneNumbers[number] ~= nil then
	local mess = _U('service_call_prefix', alert.numero, alert.message)
...
AddEventHandler('esx_phone:registerNumber', function(number, type, sharePos, hasDispatch, hideNumber, hidePosIfAnon)
    print('= INFO = Registering ' .. number .. ' => ' .. type)
...
AddEventHandler('esx_addons_gcphone:startCall', function (number, message, coords)
  ...
  if PhoneNumbers[number] ~= nil then
    ...
    print('= WARNING = Call to an unregistered service => Number : ' .. number)
  end
end)
...
AddEventHandler('esx_phone:send', function(number, message, _, coords)
  ...
  if PhoneNumbers[number] ~= nil then
    getPhoneNumber(source, function (phone) 
        ...
        print('= ERROR =  Appels sur un service non enregistre => number : ' .. number)
  end
end)